...
app.controller('ManagerController', function($scope, $http) {
	...
	$scope.newReservation = function() {
		$scope.reservations.push({label: '', mac_address: '', ip_address: '', bootfile: ''});
	}
	...
	$scope.generateFile = function() {
		...
		config += '# Please DO NOT edit this file directly!\n';
		config += '# All chganges to this file will LOST while doing a config file update!\n\n';
		config += '# Generated by https://github.com/liyvhg/isc-dhcp-configurator/\n\n';
		//config += '# Forked from https://github.com/stuartford/isc-dhcp-configurator/\n\n';
		...
		$scope.reservations.forEach(function(rsv) {
			if (rsv.label != '') {
				config += 'host '+rsv.label+' { hardware ethernet '+rsv.mac_address+'; ';
				if (rsv.ip_address != '') {
					config += 'fixed-address '+rsv.ip_address+'; ';
				}
				if (rsv.bootfile != '' && rsv.bootfile != 'undefined' ) {
					config += 'filename "'+rsv.bootfile+'"; ';
				}
				config += ' }\n';
			}
		});
	...
	/*
		// save a conf file for include
		// API call
		$http.post('api.php', {
			
			method: 'genConfigFile',
			file: $scope.loadedFile,
			parameters: $scope.parameters,
			reservations: $scope.reservations,
			content: config
		...
		//var win = window.open("data:text/plain,", "somename");
	...
	*/
	...
	$scope.previewConfig = function() {
		
		var config;
		config = $scope.generateFile();
		
		// show generated data in a new window
		window.open("data:text/plain," + escape(config));
	}
	
	// genarate the (include) dhcp config file
	$scope.genConfigFile = function() {
		...
		$http.post('api.php', {
			...
			//alert('File generate successfully.\n'+data.status+' bytes writed.\n'+'http code = '+status);
			//alert('File generate successfully.\n'+'Filename: '+data.filename+', '+data.status+' bytes writed.\n'+'http code = '+status);
			alert('File generated successfully.\n'+'Filename: '+data.filename+', '+data.status+' bytes\n');
	...
	$scope.applyConfigFile = function() {
